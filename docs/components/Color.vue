  ...
  <div>
    <div v-for="(tokenList, index) in tokens" :key="index">
      ...
      <div :class="$style.colors">
        <div v-for="(prop, index) in tokenList" :class="$style.color" :key="index">
          <div :class="$style.swatch" :style="{ backgroundColor: prop.value }" />
          <h4>{{prop.name.replace(/_/g, " ").replace(/color/g, "").replace(`${prop.category}`, "")}}</h4>
          <span>
            <em>RGB:</em>
            {{prop.value}}
          </span>
...
@import "../../src/styles/mixins/_spacing.scss";
