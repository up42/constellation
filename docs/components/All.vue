<template>
  <div :class="$style.allTokens">
    <table>
      ...
      <tbody>
        <tr v-for="(token, index) in tokens" :key="index" :class="$style.token">
          <td v-if="token.name">
            <code :class="$style.name">${{token.name.replace(/_/g, "-")}}</code>
          </td>
          <td v-else>N/A</td>
          <td v-if="token.value">
            <div
              v-if="token.type === 'color'"
              :class="[$style.example, $style.color]"
              :style="{ backgroundColor: token.value }"
            />
            <div
              v-if="token.category === 'border-radius'"
              :class="[$style.example, $style.borderRadius]"
              :style="{ borderRadius: token.value }"
            />
            <div
              v-if="token.category === 'box-shadow'"
              :class="[$style.example, $style.boxShadow]"
              :style="{ boxShadow: token.value }"
            />
            <code :class="$style.type">{{token.value}}</code>
          </td>
          <td v-else>N/A</td>
